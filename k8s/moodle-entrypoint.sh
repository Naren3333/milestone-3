...
export MOODLE_SSL_PROXY="${MOODLE_SSL_PROXY:-$MOODLE_REVERSE_PROXY}"
...
$sslProxy = getenv('MOODLE_SSL_PROXY') === '1';
$config = "<?php\nunset(\$CFG);\n";
...
if ($reverseProxy) {
    ...
    $config = preg_replace("/\\$CFG->reverseproxy\\s*=\\s*true;\\n/", '', $config);
}
if ($sslProxy) {
    ...
    $config = preg_replace("/\\$CFG->sslproxy\\s*=\\s*true;\\n/", '', $config);
}
